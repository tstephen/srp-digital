...
var ractive = new BaseRactive({
  ...
  data: {
    ...
    stdPartials: [
      ...
      { "name": "answerCriteriaSect", "url": "/srp/2.0.0/partials/answer-criteria-sect.html"},
      { "name": "answerListSect", "url": "/srp/2.0.0/partials/answer-list-sect.html"}
    ],
  ...
  fetchOrgs: function() {
    ...
    $.getJSON(ractive.getServer()+'/'+ractive.get('tenant.id')+'/accounts/', function(data) {
      ...
      for (var idx = 0 ; idx < data.length ; idx++) {
        $('datalist#orgs').append('<option value="'+data[idx].customFields['orgCode']+'">'+data[idx].name+'</option>');
      };
    });
  },
